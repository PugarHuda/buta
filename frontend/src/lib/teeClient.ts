 ...
 *
 * opType has no default on purpose. It used to default to "ORDERBOOK", which
 * the extension stopped accepting when the order book was removed — every call
 * that relied on the default would have come back "unsupported op type". Making
 * it required means the compiler asks rather than the enclave.
 */
