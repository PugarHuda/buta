...
> 1b. **Always pass both compose files.** `-f docker-compose.yaml -f
>    docker-compose.coston2.yaml`. `BUTA_FUNDS_RPC` is set only in the second
>    one, and the solvency screen turns itself OFF with an Info line when that
>    variable is missing - no error, no failed start, and clearing carries on
>    without the screen the submission claims. A bare `docker compose up -d
>    extension-tee` silently ships a different product. Check the running one
>    with `docker logs buta-extension-tee-1 | grep "solvency screening"`.
...
> volume, no seed, no env override. `docker compose -f docker-compose.yaml -f
> docker-compose.coston2.yaml up -d extension-tee` after a
