...
| ECIES-sealed bids | Opening encrypted to the enclave key **the diamond publishes** - read with `getRandomTeeIds` + `getPublicKey`, not from the operator's `/info`; the operator sees only ciphertext, and cannot substitute the key it is encrypted to | `internal/extension/decrypt.go`, `frontend/src/lib/buta.ts`, `frontend/src/lib/teeStatus.ts` |
...
| Solvency screen | The enclave passes over a bidder who could not settle - **without telling anyone what anybody bid**. It already holds the decrypted amounts, so it reads balance and allowance for the price that bidder would owe and drops them from the ranking. Only the enclave can ask that question without answering a different one | `internal/extension/solvency.go`, `pkg/auction` |
...
- **Coston2** - `ButaInstructionSender` is deployed, **source-verified on the explorer**, and **registered in the live FCC diamond** as extension **66009** (`setExtensionId` + `setTeeAddress` done on-chain). Run `node scripts/onchain-status.mjs` to confirm every line from public reads. **The full on-chain path is live**, end to end and repeatedly: `postRfq` executes, the instruction reaches the enclave (`routing action with OPType, OPCommand: BUTA, POST_RFQ` → `status 1`, empty log), the enclave signs a clearing, and `relayClearing` settles it. A **TEE machine is registered and in PRODUCTION**: `getRandomTeeIds(66009, 1)` returns `0x2F59C4cc…E83a`, and only it, instead of reverting `TooMany()`. The full product flow (post → seal ECIES bids → Vickrey clear → settle → disclose) runs on the simulated-TEE path Flare accepts.
...
- **Verified live, not just unit-tested:** enclave key read from the diamond → `ecies-geth` encrypt → 348-byte ciphertext → enclave decrypts, recovers the wallet signature, records the bid; clearing returns the winner and second price with **no hidden number anywhere in the outcome**; an unsigned or lying bid bounces with a named error.
- **Tests:** Go `pkg/auction` + `internal/extension` (full sealed lifecycle, forged sender, replayed signature, lying opening, no-leak assertion); 24 Foundry tests on the contract (the security-critical `relayClearing`, the trimmed-set rejection, signature and replay checks, `reclaimLot`).
...
- **The deployed desk reaches the enclave, so you can run the rail yourself.** Until 14 August it could not, and that was worse than it sounds: the production bundle carried no proxy URL, so the desk always fell back to its demo book - and because the book was demo, a guard correctly refused every on-chain action behind it. Anyone opening the link got a desk that could not post, seal or settle while the contract and the enclave were both live. Two things blocked it and neither is fixable in a browser: the extension proxy sends no `Access-Control-Allow-Origin` at all (Flare's own FTDC proxy does not either - we checked against it), and `/direct` needs an API key, which in a bundle is a public key. So `frontend/api/tee/` runs server-side, holds the key in an environment variable, and forwards a hard allowlist: the enclave's public key, the book, and a signed outcome. **Everything that moves money - `postRfq`, `commitBid`, `requestClearing`, `relayClearing` - is a contract call signed by your own wallet and never passes through it.** Writes over the direct channel are refused there as well as in the enclave, because a bid arriving over unauthenticated HTTP lives in enclave memory and can never be settled against the set the contract recorded. When the machine is down the desk falls back to the demo book and says so on the page - it is a container under a desk, and it behaves like one.
- **Contract (Coston2, verified):** [`0xa03821ADE58EfC07bcB1Eacd4D96ced9C7cDF74D`](https://coston2-explorer.flare.network/address/0xa03821ADE58EfC07bcB1Eacd4D96ced9C7cDF74D) · FCC extension id **66009**. Predecessor, retired 7 August: [`0x3085C895…D702`](https://coston2-explorer.flare.network/address/0x3085C89540353A4b275704b0Bd03eEc3C718D702) (extension 65642)

> Honest scope, on the record: the clearing price is public by design (Vickrey pays the second price, so that number is on-chain - what stays sealed is which bid produced it and every amount that lost). FCC itself is pre-production and being reworked on Coston2. Our machine is registered and PRODUCTION, published on a **reserved domain** (`nonmultiplicative-florida-macromolecular.ngrok-free.dev`) rather than the rotating quick tunnel it ran on until 8 August - the hostname goes on-chain, so one that changes leaves the machine unreachable and the data providers pushing at nothing. It is still a tunnel to a machine under a desk, not hosted infrastructure, and we say so.

### What we found by attacking our own code

On the last day we ran eight independent adversarial reviews across the contract, the enclave, the QA suites and the public materials. These are the findings we could not fix without redeploying a contract that already carries this submission's settlement history, so they are written down instead. Each is a real limit, not a hypothetical.

- **A bid the contract accepts but the enclave cannot open makes the two sets disagree, permanently.** `commitBid` records any 32 bytes; the enclave records only bids whose opening matches and whose signature recovers. If those differ, `setDigest != commitmentDigest(rfqId)` for every outcome the enclave will ever sign for that auction, and it can never settle. The lot is still recoverable with `reclaimLot`, so nothing is stolen, but one transaction from any address ends an auction. It is the mirror image of the property that makes trimming impossible: the same equality that rejects a smaller set rejects a larger one. The fix belongs on the enclave side, not the contract's: keep an unopenable commitment in the digest and exclude it from the ranking, so the enclave's set is always the chain's set.
- **The reserve is the one input that is neither committed nor signed nor digested.** Every bid is bound three ways; the maker's floor is only ECIES ciphertext handed to the enclave. An operator who can restart the container and replay the public `commitBid` calldata with a reserve of their choosing can binary-search the top bid from the clear-or-refuse answer alone. The honest fix is to commit the reserve on-chain and digest it alongside the commitments.
- **The solvency screen protects the winner slot, not the price.** `ClearScreened` skips a bidder who cannot pay, but the runner-up who sets the price is never screened, so a grid of zero-balance shills below the top bid raises what the real winner pays. Bids cost only gas, so this is cheap. Screening the price-setter as well is the fix, and it interacts with the deliberate choice not to reveal how many bidders were skipped.
- **`reclaimLot` has no settlement window.** Anyone may call it once the deadline passes, including after the enclave's outcome is known off-chain, which gives the maker a costless veto on a price they dislike. A grace period after the deadline, during which only a signed clearing may land, closes it.
- **`teeAddressSet` is written and never read.** Before an admin calls `setTeeAddress`, `_recover` returns the zero address for a malformed signature and the comparison passes. Not reachable on this deployment, where the address has been set since 7 August, but it is one line (`if (!teeAddressSet) revert BadTeeSignature();`) and it belongs in any redeploy.
- **The extension's action port binds all interfaces.** It sits on a shared Docker network with the operator-controlled proxy, so the provenance check that the node performs upstream can be bypassed by connecting to the port directly. Binding `127.0.0.1` is the fix.
...
| **TEE machine in PRODUCTION** | [`0x2F59C4cc8373B37c33ea61405Bf4Cf5d2F1dE83a`](https://coston2-explorer.flare.network/address/0x2F59C4cc8373B37c33ea61405Bf4Cf5d2F1dE83a), `getTeeMachineStatus` = 2. `getRandomTeeIds(66009, 1)` returns it, and only it, instead of reverting `TooMany()`. A TEE identity is ephemeral by design: `tee-node` mints a new key every start, so each replaced container registers a new machine and every predecessor - `0x473e5B49…77Ee`, `0xF9964B52…1BE6`, `0xB7b23B94…Ed7C`, `0xBDDbC0e6…d4DD`, `0x848b3D86…5Bb0` - is PAUSED. Leaving one active means a share of the instructions route to nobody and **nothing reverts to say so**: with two in the set, three of our own runs died at "no signed outcome" before `scripts/health.mjs` named the stray. It walks the whole active set rather than sampling it, and `scripts/retire-machine.mjs` pauses what it finds |
...
| **The winner is screened for solvency, in the enclave** | An empty bidder wins, `relayClearing` reverts on the `transferFrom`, and the auction dies with the runner-up who could have paid getting nothing - bidding costs nothing, so that is an attack on the maker, not a rare accident. The enclave reads balance and allowance for the price owed and drops that bidder from the ranking, revealing no amount to anyone. Live on Coston2 (`BUTA_FUNDS_RPC` in `docker-compose.coston2.yaml`); the enclave logs `solvency screening ON` at startup, and RFQ 10 settled with it on. Seven tests cover it, including that an insolvent bid cannot inflate the price it is dropped from and that a screen which cannot read the chain clears normally rather than refusing everybody |
